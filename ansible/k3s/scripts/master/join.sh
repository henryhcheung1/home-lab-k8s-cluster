...
curl -sfL https://get.k3s.io | K3S_URL="https://${MASTER_IP}:6443" K3S_TOKEN="${NODE_TOKEN}" INSTALL_K3S_EXEC="--flannel-backend=none \
--no-flannel --node-ip=${SERVER_IP} --node-external-ip=${SERVER_IP} --bind-address=${SERVER_IP} \
